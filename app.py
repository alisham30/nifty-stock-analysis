...
def recompute_from_raw(raw_dir: str, out_dir: str, shock_thr=-0.02, bounce_thr=0.0):
    """Rebuild KPIs from raw CSVs using Spark (with safe fallbacks)."""
    spark = get_spark()
    if spark is None:
    ...
    feat = (
        ...
        .withColumn("Bounce_Back",
                    F.when((F.col("Shock_Day") == 1) & (F.col("Next_Day_Return") > bounce_thr), 1).otherwise(0))
    )
...
with st.sidebar:
    ...
    st.caption("Applied to recomputation and drill-down.")
...
if not overall.empty:
    ...
    if s_col and b_col and r_col:
        ...
        c2.metric("↩️ Total bounces", f"{total_bounces:,}")
...
if not per_stock.empty:
    ...
    download_link(per_stock, "per_stock_bounce.csv", "⬇️ Download full table")

    tab1, tab2 = st.tabs(["🏆 Top 10 by bounce rate", "⬇️ Bottom 10 by bounce rate"])
    with tab1:
...
if symbols:
    ...
    spark = get_spark()
    if spark is None:
        st.warning("⚠️ Spark not available; drill-down requires Spark. Restart container and retry.")
    else:
        df = (
            spark.read.csv(f"{RAW_DIR}/*.csv", header=True, inferSchema=True)
            .withColumn("Date", F.to_date("Date"))
        ...
        if not pdf.empty:
            ...
            download_link(shocks, f"{sel}_shocks.csv", f"⬇️ Download {sel} shocks")

st.caption(f"Built with PySpark + Streamlit • Data paths: {RAW_DIR}, {OUT_DIR}")
