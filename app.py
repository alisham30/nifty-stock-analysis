# app.py — NIFTY Shock → Next-Day Bounce Dashboard (robust + clean)
...
OVERALL_PARQ = f"{OUT_DIR}/overall_bounce.parquet"
OVERALL_CSV  = f"{OUT_DIR}/overall_bounce.csv"
PER_PARQ     = f"{OUT_DIR}/per_stock_bounce.parquet"
PER_CSV      = f"{OUT_DIR}/per_stock_bounce.csv"
...
@st.cache_resource
def get_spark():
    """Create SparkSession, but don't crash the app if JVM can't start."""
    try:
        return (
...
def _pandas_read_parquet_any(path: str) -> pd.DataFrame:
    """Read a parquet dataset directory or a single parquet file using pandas/pyarrow."""
    try:
...
def read_table(parquet_path: str, csv_path: str) -> pd.DataFrame:
    """Prefer parquet: try Spark → pandas/pyarrow → finally CSV."""
...
def recompute_from_raw(raw_dir: str, out_dir: str, shock_thr=-0.02, bounce_thr=0.0):
    """
    Rebuild features and KPIs from raw CSVs using correct double precision,
    per-symbol date ordering, and duplicate (Symbol, Date) handling.
    """
    spark = get_spark()
    if spark is None:
...
if not overall.empty:
    # tolerate both total_* and num_* column names
...
if not per_stock.empty:
    ...
    with tab2:
        bottom10 = per_stock[per_stock["shock_count"] > 0] \
                   .sort_values("bounce_rate_pct").head(10)
...
if not per_stock.empty:
    ...
    # Histogram (how bounce rates are spread across stocks)
    ...
    if spark is None:
        ...
        feat = (
            ...
            .withColumn("Next_Day_Return",
                        (F.col("Next_Close") - F.col("Close")) / F.col("Close"))
            .withColumn("Shock_Day",
                        F.when(F.col("Daily_Return") <= (drop_threshold / 100.0), 1).otherwise(0))
            .withColumn("Bounce_Back",
                        F.when((F.col("Shock_Day") == 1) &
                               (F.col("Next_Day_Return") > (bounce_threshold / 100.0)), 1)
                         .otherwise(0))
            .orderBy("Date")
        )
        ...
        if not pdf.empty:
            ...
            st.dataframe(
                shocks[["Date", "Close", "Prev_Close", "Daily_Return_%",
                        "Next_Close", "Next_Day_Return_%", "Bounce_Back"]],
                use_container_width=True, height=320
            )

            # Equity curve (per selected stock): cumulative product of (1 + daily return)
...
st.caption("Built with PySpark + Streamlit • Data paths: /home/jovyan/work/{data,output}")
